...
export const useUserData = () => {
  ...
  const initializeUserData = async () => {
    ...
    try {
      ...
      const [customersRes, staffRes, servicesRes] = await Promise.all([
        ...
        supabaseClient.from('staff').select('*').eq('user_id', user.id).limit(1),
  ...
  const createSampleData = async () => {
    ...
    try {
      ...
      const { data: sampleStaff } = await supabaseClient
        .from('staff')
        .insert([
          { name: 'Main Stylist', user_id: user.id },
          { name: 'Assistant', user_id: user.id },
        ])
        .select();
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [customersRes, staffRes, servicesRes] = await Promise.all([
        supabaseClient.from('customers').select('*').eq('user_id', user.id).order('name').limit(10000),
        supabaseClient.from('staff').select('*').eq('user_id', user.id).order('name').limit(10000),
        supabaseClient.from('services').select('*').eq('user_id', user.id).order('name').limit(10000),
      ]);
